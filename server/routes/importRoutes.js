    ...
    for (let i = 0; i < tracks.length; i += batchSize) {
      ...
      const results = await Promise.all(
        batch.map(async (track) => {
          ...
          // For JioSaavn source — re-search to get a playable stream_url + jiosaavn_id
          // (the raw playlist API only gives metadata + an encrypted media URL, not a usable stream link)
          if (source === 'jiosaavn') {
            const jiosaavn = await searchJioSaavnTrack(track.title, track.artist);
            return jiosaavn
              ? { ...jiosaavn, originalTitle: track.title }
              : null;
          }
