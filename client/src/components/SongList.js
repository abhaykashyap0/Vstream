...
const SongList = ({ songs: rawSongs, queue: rawQueue, playlistId, onRemoveSong }) => {
  // Safety guard — ensure songs is always an array
  const songs = Array.isArray(rawSongs) ? rawSongs : (rawSongs?.songs || []);
  const queue = Array.isArray(rawQueue) ? rawQueue : (rawQueue?.songs || songs);
  ...
  const openPicker = async (e, songId) => {
    ...
    try {
      const { data } = await axios.get(`${API}/api/playlists/my-library`, {
  ...
  const addToExisting = async (e, songId, playlistTitle) => {
    e.stopPropagation();
    try {
      await axios.post(`${API}/api/playlists/add`,
  ...
  return (
    <>
    {/* ✅ Playlist picker as fixed modal — visible on all screen sizes */}
    {showPlaylistPicker && (
      <div
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <span style={{ fontWeight: 700, fontSize: '1rem' }}>Add to Playlist</span>
            <button onClick={() => setShowPlaylistPicker(null)}
              style={{ background: '#333', border: 'none', borderRadius: '50%', width: '30px', height: '30px', color: 'white', cursor: 'pointer', fontSize: '16px' }}>
              ✕
            </button>
          </div>
          ...
          {playlists.length > 0 && (
            <div style={{ marginBottom: '12px' }}>
              {playlists.map(pl => (
                <div
                  key={pl._id}
                  onClick={(e) => addToExisting(e, showPlaylistPicker, pl.title)}
                  style={{
          ...
          {/* New playlist */}
          <div style={{ borderTop: playlists.length > 0 ? '1px solid #333' : 'none', paddingTop: playlists.length > 0 ? '14px' : '0' }}>
            <p style={{ color: '#b3b3b3', fontSize: '0.82rem', margin: '0 0 8px' }}>Create new playlist</p>
            <input
              type="text"
              placeholder="Playlist name..."
              value={newName}
              onChange={e => setNewName(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter') { const fakeE = { stopPropagation: () => {} }; addToNew(fakeE, showPlaylistPicker); }}}
              autoFocus
              style={{
            ...
            />
            <button
              onClick={(e) => addToNew(e, showPlaylistPicker)}
              style={{
                width: '100%', padding: '12px',
                borderRadius: '8px', background: '#1db954',
...
//song playlist size and otp login and sin=gnup
