...
const PlayerBar = () => {
  ...
  const playNextRef      = useRef(null); // always holds latest playNext to avoid stale closure
        ...
        events: {
          ...
          onStateChange: (e) => {
            if (e.data === window.YT.PlayerState.PLAYING) {
              ...
              // Direct Audio pipeline initialization trigger
              silentAudioRef.current?.play().catch(() => {});
              
              // ✅ Media Session API setup to coordinate background playback listeners
...
//               stopProgressTracking();
//               setProgress(0);
//               // ✅ Destroy the player immediately — removes iframe before YouTube shows suggestions
//               try {
//                 playerRef.current.destroy();
//                 playerRef.current = null;
//                 isReady.current = false;
...
//   // ── Resume YouTube + silent audio when screen unlocks / tab refocuses ──
//   useEffect(() => {
//     const onVisible = () => {
//       if (document.visibilityState === 'visible') {
//         // Resume silent audio first
//         silentAudioRef.current?.play().catch(() => {});
//         // Then resume YouTube
//         setTimeout(() => {
//           try {
...
//       let res = await axios.get('https://lrclib.net/api/search', { params: { q: `${aggressiveTitle} ${cleanArtist}` } });
//       if (res.data?.length > 0 && processResult(scoredMatch(res.data, aggressiveTitle, cleanArtist))) return;
...
//                   title:   currentSong.title,
//                   artist:  currentSong.artist,
//                   artwork: [{ src: currentSong.image_url, sizes: '512x512', type: 'image/jpeg' }]
//                 });
//                 navigator.mediaSession.playbackState = 'playing';
...
//             }}>
//               <div ref={playerContRef} style={{ width: '100%', height: '100%' }} />
//               <div style={{ position: 'absolute', inset: 0, zIndex: 20, background: 'transparent', cursor: 'default' }}
...
//                   <span style={{ color: '#b3b3b3', fontSize: '0.78rem' }}>Sync</span>
//                   <button onClick={() => setLyricsOffset(prev => { const v = Math.round((prev - 0.5) * 10) / 10; lyricsOffsetRef.current = v; return v; })}
//                     style={{ background: '#333', border: 'none', borderRadius: '50%', width: '26px', height: '26px', cursor: 'pointer', color: 'white', fontSize: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>−</button>
...
//                     style={{ background: '#333', border: 'none', borderRadius: '50%', width: '26px', height: '26px', cursor: 'pointer', color: 'white', fontSize: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
//                   {lyricsOffset !== 1.5 && (
//                     <button onClick={() => { setLyricsOffset(1.5); lyricsOffsetRef.current = 1.5; }}
//                       style={{ background: 'transparent', border: 'none', color: '#666', cursor: 'pointer', fontSize: '0.75rem' }}>Reset</button>
//                   )}
//                 </div>
//               )}
...
//                   : <Repeat  size={18} style={{ color: repeat === 'all' ? '#1db954' : '#b3b3b3' }} />}
//                 {repeat !== 'none' && (
//                   <div style={{ position: 'absolute', bottom: '-4px', left: '50%', transform: 'translateX(-50%)', width: '4px', height: '4px', borderRadius: '50%', background: '#1db954' }} />
//                 )}
//               </div>
//             </div>
...
//   const lyricsOffsetRef  = useRef(1.5);
//   const playNextRef      = useRef(null);
//   const silentAudioRef   = useRef(null); // always holds latest playNext to avoid stale closure // mirror of lyricsOffset for use inside setInterval
...
//   // ── Resume YouTube when screen unlocks / tab refocuses ────────────────
//   useEffect(() => {
//     const onVisible = () => {
//       if (document.visibilityState === 'visible') {
//         silentAudioRef.current?.play().catch(() => {});
...
//                 // Lock screen controls
//                 navigator.mediaSession.setActionHandler('play', async () => {
//                   // Resume silent audio first — required for browser to allow playback
//                   try {
...
// import { Play, Pause, SkipBack, SkipForward, Volume2, VolumeX, X, Music, Tv, Maximize2, Minimize2, Shuffle, Repeat, Repeat1 } from 'lucide-react';
// import axios from 'axios';
// // MusicControls — uses Capacitor plugin bridge directly
// const MC = {
//   _get: () => {
//     try {
//       // Capacitor registers plugins on window.Capacitor.Plugins
//       const p = window?.Capacitor?.Plugins?.MusicControls;
//       if (p) return p;
//       // fallback: try require
//       // eslint-disable-next-line
//       const m = require('capacitor-music-controls-plugin');
//       return m?.MusicControls || m?.default?.MusicControls || m?.default || m;
//     } catch { return null; }
//   },
//   create:          (o) => { try { MC._get()?.create(o); }                         catch(e) { console.warn('MC.create:', e.message); } },
//   updateIsPlaying: (v) => { try { MC._get()?.updateIsPlaying({ isPlaying: v }); } catch(e) { console.warn('MC.update:', e.message); } },
//   destroy:         ()  => { try { MC._get()?.destroy(); }                          catch(e) { console.warn('MC.destroy:', e.message); } },
//   listen:          (cb)=> { try { return MC._get()?.addListener('controlsNotification', cb); } catch { return null; } }
// };
...
//   // ── Silent audio — keeps Media Session notification alive in background ──
//   useEffect(() => {
//     let audio = null;
//     let ctx = null;

//     const initSilentAudio = () => {
//       if (silentAudioRef.current) return; // already initialized
//       try {
//         // AudioContext approach — most reliable across browsers
...
//       silentAudioRef.current = audio;
//       // Auto-resume if browser suspends it
//       audio.addEventListener('pause', () => { audio.play().catch(() => {}); });
//     };
...
//     // Also handle visibility change — resume if suspended
//     const onVis = async () => {
//       if (document.visibilityState === 'visible') {
...
//           silentAudioRef.current.play().catch(() => {});
//         }
//         // Force resume YouTube after screen unlock
//         setTimeout(() => {
//           if (playerRef.current && isReady.current) {
//             try {
...
//       try { audio?.pause(); if (audio) audio.srcObject = null; } catch {}
//       try { ctx?.close(); } catch {}
//     };
//   }, []);
...
//   // ── Music Controls (foreground service — keeps audio alive in background) ──
//   const createMusicControls = (song, playing) => {
//     MC.create({
...
//   // Subscribe to notification controls (play/pause/next/prev buttons)
//   useEffect(() => {
//     const listener = MC.listen((action) => {
//       const msg = action?.message || action;
//       if      (msg === 'music-controls-next')     playNextRef.current?.();
//       else if (msg === 'music-controls-previous') playPrev();
//       else if (msg === 'music-controls-play')     { playerRef.current?.playVideo();  setIsPlaying(true);  }
//       else if (msg === 'music-controls-pause')    { playerRef.current?.pauseVideo(); setIsPlaying(false); }
//       else if (msg === 'music-controls-destroy')  { playerRef.current?.pauseVideo(); setIsPlaying(false); MC.destroy(); }
//     });
...
//         // Screen turned off / home button pressed
//         // Ensure silent audio is playing so Media Session stays alive
//         try { await silentAudioRef.current?.play(); } catch {}
//       } else {
//         // App came back to foreground — resume YouTube if it was paused
...
//               try { e.target.setPlaybackQuality('hd720'); } catch {}
//               // ✅ Tell OS this is a music player — keeps audio alive on lock screen
//               // Update Media Session — enables lock screen / notification controls
//               if ('mediaSession' in navigator) {
//                 navigator.mediaSession.metadata = new window.MediaMetadata({
//                   title:  currentSong.title,
//                   artist: currentSong.artist,
...
//                 });
//                 navigator.mediaSession.playbackState = 'playing';
//                 // These must be set every time to stay active
...
// import { Play, Pause, SkipBack, SkipForward, Volume2, VolumeX, X, Music, Tv, Maximize2, Minimize2, Shuffle, Repeat, Repeat1 } from 'lucide-react';
// import axios from 'axios';
// // MusicControls — safe wrapper, only active in native APK
// const MC = (() => {
//   let plugin = null;
//   const load = () => {
...
//       // eslint-disable-next-line
//       const m = require('capacitor-music-controls-plugin');
//       plugin = m?.MusicControls || m?.default?.MusicControls || m?.default || m;
//     } catch {}
//     return plugin;
//   };
//   return {
//     create:          (o) => { try { load()?.create(o); }                          catch(e) { console.warn('MC.create', e); } },
//     updateIsPlaying: (v) => { try { load()?.updateIsPlaying({ isPlaying: v }); }  catch(e) { console.warn('MC.update', e); } },
//     destroy:         ()  => { try { load()?.destroy(); }                           catch(e) { console.warn('MC.destroy', e); } },
//     listen:          (cb)=> { try { return load()?.addListener('controlsNotification', cb); } catch { return null; } }
//   };
// })();
...
//   // Re-acquire wake lock + force resume when app comes back to foreground
//   useEffect(() => {
//     const onVisibility = async () => {
//       if (document.visibilityState === 'visible') {
//         await acquireWakeLock();
//         // YouTube gets paused by Android on background — force resume
//         let attempts = 0;
...
//                 playerRef.current.playVideo();
//               }
//               clearInterval(tryResume);
//             }
//           } catch {}
//           if (attempts > 10) clearInterval(tryResume);
//         }, 300);
//       }
//     };
//     document.addEventListener('visibilitychange', onVisibility);
...
//   // Keep YouTube alive — ping player every 15s to prevent Android from suspending it
//   useEffect(() => {
//     const keepAlive = setInterval(() => {
//       try {
//         if (playerRef.current && isReady.current) {
//           const state = playerRef.current.getPlayerState?.();
//           if (state === 2 && document.visibilityState === 'hidden') {
//             // Android paused it in background — resume
//             playerRef.current.playVideo();
//           }
//         }
//       } catch {}
...
// import { Play, Pause, SkipBack, SkipForward, Volume2, VolumeX, X, Music, Tv, Maximize2, Minimize2, Shuffle, Repeat, Repeat1 } from 'lucide-react';
// import axios from 'axios';
// // MusicControls loaded dynamically to avoid build errors
// let MusicControls = null;
// try {
//   const mc = require('capacitor-music-controls-plugin');
//   MusicControls = mc.MusicControls || mc.default?.MusicControls || mc.default || mc;
// } catch {}

// // Safe wrapper — only works in APK, silently ignored in browser
// const MC = {
//   create:          (o) => { try { MusicControls.create(o); } catch {} },
//   updateIsPlaying: (v) => { try { MusicControls.updateIsPlaying({ isPlaying: v }); } catch {} },
//   destroy:         ()  => { try { MusicControls.destroy(); } catch {} },
//   listen:          (cb)=> { try { return MusicControls.addListener('controlsNotification', cb); } catch { return null; } }
// };
