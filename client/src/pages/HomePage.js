...
const HomePage = () => {
  ...
  const fetchSuggestions = async (refresh = false) => {
    ...
    try {
      ...
      const { data } = await axios.get(`${API}/api/search/suggestions`, {
        headers: getHeaders(),
        params
      });
      setSuggestions(data);
  ...
  const fetchSmartPlaylist = async () => {
    ...
    try {
      const { data } = await axios.get(`${API}/api/search/smart-playlist`, { headers: getHeaders() });
  ...
  const handleSavePlaylist = async (name, songs) => {
    ...
    await axios.post(`${API}/api/playlists/create`, { title: name }, { headers: getHeaders() });
    await Promise.all(songs.map(song =>
      axios.post(`${API}/api/playlists/add`,
